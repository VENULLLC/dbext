...
LDFLAGS = -Wl,-rpath,'$$ORIGIN' -Llibb64-1.2/src -L$(DBEXT_BUILD_DIR)/uuid/lib -lb64 -luuid
...
libb64-1.2/src/libb64.a: libb64-1.2.src.zip
...
$(DBEXT_BUILD_DIR)/uuid/lib/libuuid.a: libuuid-1.0.3.tar.gz
	cd libuuid-1.0.3 && ./configure --with-pic --disable-shared --enable-static --prefix=$(DBEXT_BUILD_DIR)/uuid --host=$(if $(TARGET_TUPLE),$(TARGET_TUPLE),$(shell gcc -dumpmachine)) && CFLAGS=-fPIC $(MAKE) && $(MAKE) install

$(DBEXT_BUILD_DIR)/$(OUT): $(FINAL_OBJECTS) libb64-1.2/src/libb64.a $(DBEXT_BUILD_DIR)/uuid/lib/libuuid.a
	@echo $(SOURCES)
